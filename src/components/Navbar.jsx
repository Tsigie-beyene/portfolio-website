    ...
    border-r border-gray-200 px-4 py-10 z-10 ">
        ...
        <div className="flex flex-col gap-y-3 sm:gap-y-2">
            {navbarData.map((item, i) => (
              ...
              group-hover:opacity-100 transition-all duration-300 text-center ${i% 2 ===0 ? "translate-x-2": "-translate-x-2"}
        ...
        <p className="flex items-center justify-center text-[13px] text-gray-600 mt-6">
            <span className="absolute left-1/2 w-max flex items-center -rotate-90 origin-bottom-left tracking-wider">{copyRightIcon} 2020 - {new Date().getFullYear()}</span>
        </p>
    </div>
  )
}
